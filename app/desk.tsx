...
import { newestBatch, inCollection, batchOptions } from '@/lib/collections';
...
export default function Desk({leads}:{leads:Lead[]}) {
 const [selected,setSelected]=useState((leads.find(l=>l.batch===newestBatch&&l.status==='ready')||leads.find(l=>l.batch===newestBatch)||leads[0]).id);
 const [collection,setCollection]=useState(newestBatch);
 const scoped=leads.filter(l=>inCollection(l,collection));
 const collections=batchOptions(leads);
 ...
 const eligible=readyEmails(scoped,states);
 useEffect(()=>{
 ...
 <section className="intro"><div><p>INDIA + USA / BUSINESS OUTREACH</p><h1>Your next conversation.</h1><span>Written for each business. Ready for your review.</span></div><div className="counts"><div><strong>{scoped.filter(l=>state(l)==='ready').length}</strong><span>Ready</span></div><div><strong>{scoped.filter(l=>state(l)==='sent').length}</strong><span>Sent</span></div><div><strong>{scoped.filter(l=>state(l)==='bounced').length}</strong><span>Bounced</span></div></div></section>
 ...
 <section className="setup" aria-label="Bulk email sending"><h2>Send all ready emails</h2><p>Each business receives its own prepared message. Sent, bounced, blocked and unverified contacts are excluded. Includes ready contacts in the selected batch, not just the search results. Selected batch: {scoped.filter(l=>l.country==='India').length} India + {scoped.filter(l=>l.country==='USA').length} USA records. {scoped.filter(l=>l.status!=='ready').length} are held and cannot be sent. A directory listing does not prove that a business has no website. Public emails are not delivery-verified.</p><Button onClick={sendAll} disabled={busy||!loaded||!connected||!eligible.length}><Send/>Send all ({eligible.length})</Button>{batch?<><p role="status">{progress.sent} / {progress.total} accepted by Gmail. Keep this tab open.</p><Button variant="outline" onClick={()=>{stopBatch.current=true;setNotice('Stopping after the current request. Already accepted emails cannot be recalled.');}}>Stop sending</Button></>:null}</section>
 {!loaded?<div className="notice">Saved history is not loaded. <button onClick={refresh}>Reload history</button></div>:null}
<div className="toolbar"><select aria-label="Choose batch" value={collection} disabled={busy} onChange={e=>{setCollection(e.target.value);setFilter('all');setSearch('');const first=leads.find(l=>inCollection(l,e.target.value));if(first)setSelected(first.id);}}>{collections.map(c=><option key={c.value} value={c.value}>{c.label}</option>)}</select><Input aria-label="Find a business" placeholder="Find a business…" value={search} onChange={e=>setSearch(e.target.value)}/><Select value={filter} onValueChange={v=>v&&setFilter(v)}><SelectTrigger aria-label="Filter businesses"><SelectValue/></SelectTrigger><SelectContent>{[['ready','Ready to send'],['sent','Sent'],['review','Needs attention'],['all',`All ${scoped.length} businesses`]].map(([v,t])=><SelectItem key={v} value={v}>{t}</SelectItem>)}</SelectContent></Select><span>{visible.length} businesses</span></div>
 <div className="workspace"><section className="lead-list" aria-label="Businesses">{visible.length?visible.map(l=><button className={'lead '+(selected===l.id?'selected':'')} key={l.id} onClick={()=>setSelected(l.id)} aria-pressed={selected===l.id}><span className="badge">{statusLabel(state(l))}</span><h2>{l.business}</h2><p>{l.email}</p><small>{l.country||'India'}</small></button>):<p>No businesses match this view.</p>}</section>
 ...
 {lead.reason?<div className="hold"><p>{lead.reason}</p></div>:null}
 ...
 <details className="evidence"><summary>Website finding &amp; source</summary><p>{lead.evidence||'The earlier finding could not be reconfirmed. Do not treat it as verified.'}</p><a href={lead.source} target="_blank" rel="noreferrer">View source page <ArrowUpRight size={14}/></a>{lead.corroboration?<p><a href={lead.corroboration} target="_blank" rel="noreferrer">Additional business listing ↗</a></p>:null}<p className="fine">Research checked {lead.checked||'date unavailable'}. A text excerpt can include hidden or alternate page content. Published email addresses are not a delivery guarantee.</p></details>
