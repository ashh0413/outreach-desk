...
export default function Desk({leads}:{leads:Lead[]}) {
 const [selected,setSelected]=useState((leads.find(l=>l.batch&&l.status==='ready')||leads[0]).id);
 const [collection,setCollection]=useState('fresh');
 const scoped=leads.filter(l=>collection==='fresh'?!!l.batch:!l.batch);
 ...
 <section className="setup" aria-label="Bulk email sending"><h2>Send all ready emails</h2><p>Each business receives its own prepared message. Sent, bounced, blocked and unverified contacts are excluded. Includes ready contacts in the selected batch, not just the search results. The new batch has 50 India and 50 USA prospects; 46 remain held for verification. Public emails are not delivery-verified.</p><Button onClick={sendAll} disabled={busy||!loaded||!connected||!eligible.length}><Send/>Send all ({eligible.length})</Button>{batch?<><p role="status">{progress.sent} / {progress.total} accepted by Gmail. Keep this tab open.</p><Button variant="outline" onClick={()=>{stopBatch.current=true;setNotice('Stopping after the current request. Already accepted emails cannot be recalled.');}}>Stop sending</Button></>:null}</section>
 {!loaded?<div className="notice">Saved history is not loaded. <button onClick={refresh}>Reload history</button></div>:null}
<div className="toolbar"><select aria-label="Choose batch" value={collection} disabled={busy} onChange={e=>{setCollection(e.target.value);setFilter('all');setSearch('');const first=leads.find(l=>e.target.value==='fresh'?!!l.batch:!l.batch);if(first)setSelected(first.id);}}><option value="fresh">New: India 50 + USA 50</option><option value="historical">Earlier Bengaluru batch</option></select><Input aria-label="Find a business" placeholder="Find a business…" value={search} onChange={e=>setSearch(e.target.value)}/><Select value={filter} onValueChange={v=>v&&setFilter(v)}><SelectTrigger aria-label="Filter businesses"><SelectValue/></SelectTrigger><SelectContent>{[['ready','Ready to send'],['sent','Sent'],['review','Needs attention'],['all',`All ${scoped.length} businesses`]].map(([v,t])=><SelectItem key={v} value={v}>{t}</SelectItem>)}</SelectContent></Select><span>{visible.length} businesses</span></div>
